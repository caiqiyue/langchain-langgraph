# <center>第四阶段、实战大模型 - 简单问答机器人</center>
...
![Gradio对话机器人架构图](../assets/第四课_01_Gradio对话机器人架构.html)

**图解说明**：Gradio 对话机器人架构 — 用户输入 → 消息构建(HumanMessage) → LangChain模型流式输出 → 聊天历史展示 → State状态保存；核心组件：Chatbot、Textbox、State
