## <center>第四阶段、GraphRAG核心架构</center>
...
![GraphRAG索引流程图](https://muyu20241105.oss-cn-beijing.aliyuncs.com/images/202411201153594.png)
...
![GraphRAG查询流程图](https://muyu20241105.oss-cn-beijing.aliyuncs.com/images/202411201203487.png)
...
&emsp;&emsp;**GraphRAG索引过程中各阶段的作用？**

&emsp;&emsp;GraphRAG索引过程包含多个关键阶段：
- **文本分块**：将大文档分成小的TextUnits，便于处理
- **实体提取**：识别文本中的关键实体
- **关系抽取**：建立实体间的语义关系
- **社区检测**：使用Leiden算法识别紧密连接的实体群
- **分层摘要**：生成不同级别的社区摘要

&emsp;&emsp;**为什么需要社区检测算法？**

&emsp;&emsp;社区检测在GraphRAG中至关重要：
- **全局理解**：帮助理解整个数据集的主题结构
- **高效检索**：通过社区而非单个实体进行检索，提高效率
- **多级粒度**：不同级别的社区提供不同详细程度的信息

&emsp;&emsp;**Leiden算法相比Louvain的优势？**

&emsp;&emsp;Leiden算法是Louvain的改进版本：
- **更快的收敛速度**：能够更快地找到最优社区划分
- **更好的质量保证**：产生的社区更加紧密和有意义
- **避免盲目迭代**：能够识别出不稳定的社区结构

&emsp;&emsp;**查询阶段如何选择合适的社区级别？**

&emsp;&emsp;社区级别的选择取决于查询类型：
- **高层级（粗糙）**：适用于需要全局理解的查询
- **中层级**：适用于需要平衡广度和深度的查询
- **低层级（精细）**：适用于需要具体细节的查询
