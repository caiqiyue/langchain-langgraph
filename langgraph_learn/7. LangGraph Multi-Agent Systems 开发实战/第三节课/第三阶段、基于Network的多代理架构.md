## <center>第三阶段、基于 Network（网络）的多代理架构
...
# 数据库连接
DATABASE_URI = 'mysql+pymysql://root:password@host/langgraph_agent?charset=utf8mb4'
...
![数据库表结构](https://muyu20241105.oss-cn-beijing.aliyuncs.com/images/202411151517135.png)
...
![数据验证](https://muyu20241105.oss-cn-beijing.aliyuncs.com/images/202411151542873.png)
...
def add_sale(product_id: int, employee_id: int, customer_id: int, sale_date: str,
             quantity: int, amount: float, discount: float):
...
def delete_sale(sales_id: int):
    ...
    try:
        ...
        if sale_to_delete:
            ...
            session.commit()
            return f"Sale record {sales_id} deleted successfully."
        else:
            return f"Sale record {sales_id} not found."
    except Exception as e:
        return f"Failed to delete sale: {e}"
    finally:
        session.close()
...
def update_sale(sales_id: int, quantity: int, amount: float):
    ...
    try:
        ...
        if sale_to_update:
            ...
            session.commit()
            return f"Sale record {sales_id} updated successfully."
        else:
            return f"Sale record {sales_id} not found."
    except Exception as e:
        return f"Failed to update sale: {e}"
    finally:
        session.close()
...
def query_sales(sales_id: int):
    """Query a sale record by sales_id."""
    session = Session()
    try:
        ...
        return f"Failed to query sale: {e}"
    finally:
        session.close()
```
...
**Network 架构与 Supervisor 架构的核心区别？**

Network 架构采用去中心化的设计，每个代理都可以直接与其他代理通信（多对多连接），而 Supervisor 架构采用中央控制模式，由 Supervisor 统一管理任务分配和路由。这种设计差异带来了以下区别：
- **通信模式**：Network 中代理之间可以直接通信，而 Supervisor 中所有通信都通过主管节点
- **灵活性**：Network 更加灵活，但难以控制；Supervisor 更加可控，但可能成为瓶颈
- **扩展性**：Network 随代理数量增加难以维护，Supervisor 更容易扩展新代理

**为什么需要动态路由？**

在 Network 架构中，路由器（router）的核心作用是根据代理的输出决定下一步应该调用哪个代理或结束流程。动态路由的优势：
- **适应性**：可以根据实时状态做出决策
- **效率**：避免不必要的代理调用
- **可控性**：确保任务按预期流程执行
...
ToolNode 是 LangGraph 封装的工具执行器，它：
- 接收工具调用列表
- 自动执行对应的工具函数
- 返回工具执行结果

在多代理系统中，ToolNode 让代理能够调用外部工具完成具体任务。

### 2. 实践应用

**何时使用 Network 架构而不是 Supervisor 架构？**

当您的应用场景满足以下条件时，Network 架构可能更合适：
- 代理之间需要频繁直接通信
- 任务流程不固定，需要动态决定下一步
- 代理数量较少（少于 10 个）

当需要集中控制、流程清晰时，Supervisor 架构更合适。

**如何处理 Network 架构中的通信循环？**

使用 recursion_limit 参数限制最大迭代次数，防止无限循环：
```python
for chunk in graph.stream(
    {"messages": [HumanMessage(content="...")]},
    {"recursion_limit": 50},  # 限制最大迭代次数
    stream_mode='values'
):
    ...
```
...
**Multi-Agent 系统的状态管理挑战**

在复杂的多代理系统中，状态管理变得更加重要：
- 如何追踪每个代理的上下文？
- 如何避免状态冲突？
- 如何支持代理间的状态共享？

这需要仔细设计状态模式，确保信息正确传递。
