## <center>第二阶段、Multi-agent架构与Subgraphs</center>

&emsp;&emsp;在第一阶段中，我们已经深入学习了`Multi-Agent Systems`的基本概念、架构设计以及`Subgraphs`子图的通信原理。本阶段我们将通过**基于 Network（网络）的多代理架构**的实战案例，进一步掌握多代理系统的构建方法。

### 1. 基于 Network（网络）的多代理架构
...
![Network多代理架构图](https://muyu20241105.oss-cn-beijing.aliyuncs.com/images/202411111121494.png)
...
&emsp;&emsp;这里我们为`db_agent`配置的能力是操作数据库权限，以下是我们即将构建的数据模型（`SalesData`, `CustomerInformation`, `ProductInformation`, `CompetitorAnalysis`）表的描述，用来构成销售和市场分析核心数据的存储。
...
&emsp;&emsp;接下来，创建模拟数据。我们使用 `Python` 的 `faker` 库来生成模拟数据。
...
&emsp;&emsp;然后，第二个数据分析师（code_agent）在需要的时候，接收`db_agent`的数据，生成可视化的图表，这里我们给他配置一个本地的`Python`代码解释器。这里我们使用`Python REPL` 工具。
...
&emsp;&emsp;**要通过图状态进行通信，需要将单个代理定义为图节点，在图执行的每个步骤中，代理节点接收图的当前状态，执行代理代码，然后将更新的状态传递给下一个节点。代理节点是共享单个状态架构的**。这里我们定义一个辅助函数`create_agent`，用来帮助我们创建多代理系统中的每一个子代理。每个子代理可以通过`llm`参数定义其使用的大模型，`system_message`用于定义其每个代理独有的背景信息，而`tools`则可以给不同的子代理绑定特有的工具。
...
def agent_node(state, agent, name):
    ...
    if isinstance(result, ToolMessage):
        ...
        # 创建一个 AIMessage 类的新实例，其中包含 result 对象的所有数据
...
&emsp;&emsp;在初始化 `StateGraph` 对象之前，我们首先定义 `AgentState`。`AgentState`定义在图中的节点之间传递的对象的结构。在这里，它包括消息列表和要执行任务的代理名称(sender)。
...
&emsp;&emsp;**Network架构中的路由决策机制是如何工作的？**

&emsp;&emsp;在Network多代理架构中，路由决策是实现代理间通信的关键。每个代理完成工作后会返回一个状态，路由器根据这个状态决定下一步应该：
- 调用工具（call_tool）
- 继续传递给另一个代理（continue）
- 结束流程（END）

&emsp;&emsp;**条件边在多代理系统中的作用是什么？**

&emsp;&emsp;条件边允许动态路由，使得图的执行路径可以根据当前状态来决定。这对于多代理系统尤为重要，因为：
- 不同的代理可能处理不同类型的任务
- 需要根据任务进展动态选择下一步
- 可以实现复杂的控制流逻辑

&emsp;&emsp;**Network架构与Supervisor架构的核心区别？**

| 特性 | Network架构 | Supervisor架构 |
|------|------------|---------------|
| 通信模式 | 多对多，任何代理可与其他代理通信 | 一对多，代理与Supervisor通信 |
| 控制权 | 分布式，每个代理独立决策 | 集中式，Supervisor统一决策 |
| 扩展性 | 随代理数量增加而复杂 | 相对容易扩展 |
| 适用场景 | 灵活的协作任务 | 需要中央调度的任务 |

&emsp;&emsp;**在实际应用中如何选择合适的多代理架构？**

&emsp;&emsp;选择合适的多代理架构需要考虑：
- **任务复杂度**：简单任务可用Network，复杂任务适合Supervisor
- **控制需求**：需要严格控制流程选Supervisor，需要灵活性选Network
- **代理数量**：代理数量少可用Network，数量多建议Supervisor
- **团队熟悉度**：团队对集中式管理更熟悉选Supervisor
